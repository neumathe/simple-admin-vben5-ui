...
import { deleteMcmsBanner, getMcmsBannerList } from '#/api/mcms/mcmsBanner';
import { type ActionItem, TableAction } from '#/components/table/table-action';

import McmsBannerForm from './form.vue';
import { searchFormSchemas, tableColumns } from './schemas';

defineOptions({
  name: 'McmsBannerManagement',
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: McmsBannerForm,
});
...
const gridOptions: VxeGridProps<McmsBannerInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getMcmsBannerList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteMcmsBanner({
    ids,
  });
  if (result.code === 0) {
