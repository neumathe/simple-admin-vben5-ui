...
import {
  deleteCommunityNote,
  getCommunityNoteList,
} from '#/api/tmp/communityNote';
import { type ActionItem, TableAction } from '#/components/table/table-action';

import CommunityNoteForm from './form.vue';
import { searchFormSchemas, tableColumns } from './schemas';

defineOptions({
  name: 'CommunityNoteManagement',
});

// ---------------- form -----------------

const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: CommunityNoteForm,
});
...
const gridOptions: VxeGridProps<CommunityNoteInfo> = {
  checkboxConfig: {
    highlight: true,
  },
  toolbarConfig: {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        const res = await getCommunityNoteList({
...
async function batchDelete(ids: any[]) {
  const result = await deleteCommunityNote({
    ids,
  });
  if (result.code === 0) {
